...
def preprocess_and_rename_audio(input_folder, output_folder, min_duration, max_duration, disable_filter):
    if disable_filter:
        ...
        filter_audio(input_folder, output_folder, min_duration, max_duration)
        filter_result = f"音频过滤完成,过滤出 {len(os.listdir(output_folder))} 个文件,结果保存在 {output_folder} 文件夹中。"
...
def run_end_to_end_pipeline(input_folder, min_duration, max_duration, model_revision, batch_size, max_workers, disable_text_emotion, disable_filter):
    ...
    classify_result = classify_audio_emotions(output_file, audio_folder, max_workers, output_folder)
    return f"{preprocess_result}\n{recognize_result}\n{classify_result}"

def launch_ui():
    create_folders(["input", "referenceaudio", "csv_opt", "output"])

    with gr.Blocks(theme=gr.themes.Default(), title="音频情感识别与分类应用") as demo:
        gr.Markdown("# 🎙️ 音频情感识别与分类\n这个应用可以帮助你对音频文件进行情感识别和分类。")
        
        max_workers = gr.Slider(1, 16, 4, step=1, label="最大工作线程数")
        ...
        with gr.Tab("一键推理"):
            ...
            one_click_button.click(run_end_to_end_pipeline, [one_click_input_folder, one_click_min_duration, one_click_max_duration, one_click_model_revision, one_click_batch_size, max_workers, one_click_disable_text_emotion, one_click_disable_filter], one_click_result)
        ...
        with gr.Tab("音频情感识别"):
            ...
            recognize_button.click(recognize_audio_emotions, [recognize_folder, model_revision, batch_size, max_workers, disable_text_emotion, output_file], recognize_result)
